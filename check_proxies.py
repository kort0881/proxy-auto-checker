        ...
        for url in urls:
            try:
                ...
                for line in r.text.splitlines():
                    # FIX 1: нормализуем ключ (убираем #comment) до дедупликации и сохранения
                    ...
                    if n in seen:
                        continue
                    seen.add(n)
                    all_keys.append(n)  # сохраняем нормализованный ключ, а не сырую строку
                    count += 1
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG.TCP_WORKERS) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            if _stop_flag.is_set() or is_time_exceeded():
                break
            try:
                # FIX 3: убран timeout=0.1 — future уже готов после as_completed,
                # timeout=0.1 мог бросить TimeoutError и потерять результат
                r = fut.result()
                if r:
                    tcp_alive.append(r)
            except Exception:
