...
def generate_readme(stats_data: Dict):
    """Generate README.md with statistics and badges"""
    
    total = stats_data['total_working']
    elite = stats_data['by_quality'].get('elite', 0)
    premium = stats_data['by_quality'].get('premium', 0)
...
| Quality | Count | File |
|---------|-------|------|
| Elite   | {stats_data['by_quality'].get('elite', 0)} | [elite.txt](results/premium/elite.txt) |
| Premium | {stats_data['by_quality'].get('premium', 0)} | [premium.txt](results/premium/premium.txt) |
| Good    | {stats_data['by_quality'].get('good', 0)} | [good.txt](results/premium/good.txt) |
| Standard | {stats_data['by_quality'].get('standard', 0)} | [standard.txt](results/premium/standard.txt) |

## 🔧 Protocol Distribution

"""
    
    for protocol, count in sorted(stats_data['by_protocol'].items(), key=lambda x: x[1], reverse=True):
        percentage = (count / total * 100) if total > 0 else 0
        readme_content += f"- **{protocol}**: {count} ({percentage:.1f}%)\n"
...
- 🧬 **Smart Mutations** - {stats_data['mutations']['successful']} successful bypasses
- ✅ **Multi-layer validation** with reconnection tests
- 📈 **Latency optimization** with jitter analysis
- 🌍 **Multi-region support** with targeted sources

## 📥 Usage

```bash
# Download all elite proxies
curl -sL https://raw.githubusercontent.com/yourusername/yourrepo/main/results/premium/elite.txt

# Download specific quality
wget https://raw.githubusercontent.com/yourusername/yourrepo/main/results/premium/premium.txt
    readme_content += f"""
...
# Get elite proxies
response = requests.get('https://raw.githubusercontent.com/yourusername/yourrepo/main/results/premium/elite.txt')
proxies = [line.strip() for line in response.text.split('\\n') if line.strip() and not line.startswith('#')]
```

📝 Changelog

{datetime.now().strftime('%Y-%m-%d')}: Checked {stats_data['total_checked']} proxies, {stats_data['total_working']} working
AI Status: {'Enabled ✅' if stats_data['ai']['enabled'] else 'Disabled ❌'}
Processing Time: {stats_data['processing_time']:.1f} seconds

Generated by AI Proxy Checker v3.1 | Channel: {MY_CHANNEL}
"""

    readme_path = RESULTS_FOLDER.parent / "README.md"
    with open(readme_path, 'w', encoding='utf-8') as f:
        f.write(readme_content)

    log("📝 README generated", "info")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="AI Proxy Checker Pro - Advanced proxy validation with AI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )

    parser.add_argument(
        '--region',
        choices=['ALL', 'RU', 'EU', 'US'],
        default='ALL',
        help='Region to check (default: ALL)',
    )

    parser.add_argument(
        '--workers',
        type=int,
        default=CONFIG.XRAY_WORKERS,
        help=f'Number of xray workers (default: {CONFIG.XRAY_WORKERS})',
    )

    parser.add_argument(
        '--tcp-workers',
        type=int,
        default=CONFIG.TCP_WORKERS,
        help=f'Number of TCP workers (default: {CONFIG.TCP_WORKERS})',
    )
    ...
    parser.add_argument(
        ...
        help=f'TCP timeout in seconds (default: {CONFIG.TCP_TIMEOUT})',
    )

    parser.add_argument(
        '--samples',
        type=int,
        default=CONFIG.LATENCY_SAMPLES,
        help=f'Number of latency samples (default: {CONFIG.LATENCY_SAMPLES})',
    )
...
def main() -> int:
    ...
    CONFIG.LATENCY_SAMPLES = args.samples
    ...
    # Banner
    log("=" * 60, "info")
    log("🔥 AI PROXY CHECKER PRO v3.1", "success")
    log(f"📡 Channel: {MY_CHANNEL}", "info")
    log(f"🌍 Region: {args.region}", "info")
    log(f"🧠 AI: {'Enabled' if ai_engine.enabled else 'Disabled'}", "info")
    log("=" * 60, "info")

    # Setup xray
    log("\n📦 Setting up Xray-core...", "info")
    xray_exe = setup_xray()
    if not xray_exe:
        log("❌ Failed to setup Xray-core", "error")
        return 1
    ...
    # Download keys
    log("\n📥 Downloading proxy keys...", "info")
    all_keys = download_and_deduplicate(sources)
    if not all_keys:
        log("❌ No proxy keys found", "error")
        return 1

    # AI prioritization
    if ai_engine.enabled:
        log("\n🧠 AI prioritizing keys...", "info")
        all_keys = ai_engine.prioritize_keys(all_keys)

    # Phase 1: TCP Check
    log(f"\n🔍 Phase 1: TCP connectivity check ({CONFIG.TCP_WORKERS} workers)...", "info")
    ...
    if TQDM_AVAILABLE:
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG.TCP_WORKERS) as executor:
            ...
            for future in tqdm(
                ...
                if len(tcp_alive) % CONFIG.GC_EVERY == 0:
                    gc.collect()
    else:
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG.TCP_WORKERS) as executor:
            futures = {executor.submit(tcp_check, key): key for key in all_keys}
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if done % CONFIG.GC_EVERY == 0:
                    gc.collect()
                    log(f"[TCP] Progress: {done}/{len(all_keys)}, alive: {len(tcp_alive)}", "debug")

    log(f"✅ TCP alive: {len(tcp_alive)} / {len(all_keys)}", "success")
    if not tcp_alive:
        log("❌ No proxies passed TCP stage", "error")
        return 1

    # Phase 2: XRAY + AI mutations
    log(f"\n⚡ Phase 2: XRAY + AI ({CONFIG.XRAY_WORKERS} workers)...", "info")
    ...
    if TQDM_AVAILABLE:
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG.XRAY_WORKERS) as executor:
            futures = {
                executor.submit(xray_check_with_mutations, key, xray_exe): key
                for key in tcp_alive
            }
            for future in tqdm(
                concurrent.futures.as_completed(futures),
                total=len(futures),
                ...
                res = future.result()
                ai_engine.analyze_result(res)
                ai_engine.save_result(res)
                if res.alive:
                    results.append(res)
                    mut_tag = f" 🧬 {res.mutation_used}" if res.mutation_used else ""
                    log(
                        f"✓ {res.quality.name if res.quality else 'OK'} | "
                        f"{int(res.latency)}ms | {res.protocol}{mut_tag} {res.ai_verdict}",
                        "success",
                    )
    else:
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG.XRAY_WORKERS) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if done % 10 == 0:
                    gc.collect()
                    log(f"[XRAY] Progress: {done}/{len(tcp_alive)}, alive: {len(results)}", "debug")

    if results:
        save_results(results)
    else:
        log("⚠️ No alive proxies after XRAY stage", "warning")
